...
const { shape: shape1, string: string2 } = mod;
const noPropertiesFound1 = {
    result: {
        version: 2,
        data: {
            "Not Ready": {
...
const configMatcher = shape1({
    "tor-address": string2,
    "lan-address": string2
});
const properties1 = async (effects)=>{
    if (await mod2.exists(effects, {
        volumeId: "main",
        path: "start9/config.yaml"
    }) === false) {
        return noPropertiesFound1;
    }
    const config = configMatcher.unsafeCast(mod1.parse(await effects.readFile({
        path: "start9/config.yaml",
        volumeId: "main"
    })));
    const properties = {
...
const setConfig1 = async (effects, newConfig)=>{
    return mod4.setConfig(effects, newConfig);
};
...
const migration = mod4.migrations.fromMapping({}, "0.12.0");
const health = {
...
const healthWeb = async (effects, duration)=>{
    ...
    return await effects.fetch("http://ipfs.embassy:5001/webui").then((_)=>ok1).catch((e)=>{
        effects.error(`${e}`);
        return error1(`The IPFS Web UI is unreachable`);
    });
};
const guardDurationAboveMinimum = (input)=>input.duration <= input.minimumTime ? Promise.reject(errorCode1(60, "Starting")) : null;
const errorCode1 = (code, error)=>({
        "error-code": [
            code,
            error
        ]
